...
for (my $i=0; $i < scalar(@config); $i++) {
  ...
  # We do not search for "time" section, since this
  # section has to be created AFTER all time sections

  if ($config[$i]->{'sectype'} eq "source") {
    $line = $config[$i]->{'line'};
    last;

  } elsif ($config[$i]->{'sectype'} eq "dest") {
    $line = $config[$i]->{'line'};
    last;
...
if ($line < 0) {
  ...
  push(@$conf, "", "source $in{'name'} {", "}");
} else {
  # we found a section we want to prepend our
  # new section to

  my @newlines=( "source $in{'name'} {",
                 "}",
                 "",
                 $conf->[$line] );
