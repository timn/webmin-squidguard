...
my $st=$sg->{'members'}->[$in{'index'}] if (! $in{'new'});


&header($in{'new'} ? $text{'eshost_titleadd'} : $text{'eshost_title'}, undef, undef, undef, undef, undef,
        "Written by<BR>Tim Niemueller<BR><A HREF=http://www.niemueller.de>Home://page</A>");
print "<HR><BR>";

print "<FORM ACTION=\"save_sourcehost.cgi\" METHOD=POST>\n",
      ...
      " <TR>\n",
      "  <TD $tb WIDTH=100%><B>",
      &text($in{'new'} ? 'eshost_headeradd' : 'eshost_header', $st->{'ip'}, $in{'sourcegroup'}),
      "</B></TD>\n",
      " </TR>\n",
      " <TR>\n",
      "  <TD>\n",
      ...
      "    <TR>\n",
      "     <TD><B>$text{'eshost_ip'}</B></TD>\n",
      "     <TD><INPUT TYPE=text NAME=ip VALUE=\"", $in{'new'} ? "" : $st->{'ip'},
      "\" SIZE=15 MAXSIZE=15> ",
      "     </TD>\n",
      "    </TR>\n",
...
&footer("edit_sourcegroup.cgi?sourcegroup=$in{'sourcegroup'}", $text{'eshost_return'});

### END of edit_sourcehost.cgi ###.