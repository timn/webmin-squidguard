...
&header($in{'new'} ? $text{'esrange_titleadd'} : $text{'esrange_title'}, undef, undef, undef, undef, undef,
        "Written by<BR>Tim Niemueller<BR><A HREF=http://www.niemueller.de>Home://page</A>");
print "<HR><BR>";

print "<FORM ACTION=\"save_sourcerange.cgi\" METHOD=POST>\n",
      ...
      " <TR>\n",
      "  <TD $tb WIDTH=100%><B>",
      &text($in{'new'} ? 'esrange_headeradd' : 'esrange_header', "$st->{'ip'}-$st->{'end'}", $in{'sourcegroup'}),
      "</B></TD>\n",
      " </TR>\n",
      " <TR>\n",
      "  <TD>\n",
      ...
      "    <TR>\n",
      "     <TD><B>$text{'esrange_stop'}</B></TD>\n",
      "     <TD><INPUT TYPE=text NAME=end VALUE=\"",
      $in{'new'} ? "" : $st->{'end'}, "\" SIZE=15 MAXSIZE=15> ",
      "     </TD>\n",
      "    </TR>\n",
...
&footer("edit_sourcegroup.cgi?sourcegroup=$in{'sourcegroup'}", $text{'esrange_return'});

### END of edit_sourcerange.cgi ###.