...
} elsif (! ($config{'squiduid'} || $config{'squidgid'})) {
  # UID/GID of squid are not yet set

  print "<H3>$text{'index_nougidhead'}</H3>\n",
        "$text{'index_nougiddesc'}",
        "<BR><FORM ACTION=save_config.cgi>",
        "$text{'index_user'}: <INPUT TYPE=text NAME=squiduid SIZE=20> ",
        &user_chooser_button('squiduid'),
        "<BR>$text{'index_group'}: <INPUT TYPE=text NAME=squidgid SIZE=20> ",
        &group_chooser_button('squidgid'),
        "<BR><INPUT TYPE=submit NAME=save VALUE=\"$text{'save'}\">",
        "</FORM>";
  &footer();
