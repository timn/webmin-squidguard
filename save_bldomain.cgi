...
@config = &parse_config();
&error_setup($text{'sbd_error'});

my $dbsec=&find_section( 'config' => \@config,
                         'sectype' => 'dbhome' );
my $dbhome=$dbsec->{'dbhome'};
my $filename="$dbhome/blacklists/$in{'blacklist'}/domains";
...
if (defined($in{'delete'})) {
  ...
  # Check input
  &terror('sbd_invalid') if ($in{'domain'} !~ /^\S+$/);
...
if (defined($in{'new'}) || defined($in{'delete'})) {
  ...
  &redirect("edit_bldomain.cgi?blacklist=$in{'blacklist'}&index=$in{'index'}");
}



### END of save_bldomain.cgi ###.
